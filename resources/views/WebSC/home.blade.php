        ...
        <div class="card article">
          <div class="card-content">
              ...
              <div class="media-content has-text-centered" {!! !$author->image ? 'style="margin-top:2rem;overflow:unset;"' : '' !!}>
                @if ($post->cover)
                  <figure class="image is-3by1">
                    <a href="{{ url(config('path.post', 'post') . '/' . $post->slug) }}" title="{{ $post->title . ' - ' . app('site')->name }}">
                ...
                <div class="tags has-addons level-item">
                  <span class="tag is-rounded is-danger" title="Penulis: {{ $author->fullname }}">
                    <a href="{{ url(config('path.author', 'author') . '/' . $author->name) }}" title="Penulis: {{ $author->fullname }}" class="has-text-white">
                      {{ $author->fullname }}
                    </a>
                  </span>
            ...
            <div class="content article-body">
              {{ $post->excerpt }}
              <br><br>
              <a href="{{ url(config('path.post', 'post') . '/' . $post->slug) }}" title="{{ $post->title .' - ' . app('site')->name }}" class="button is-danger">Baca lengkap</a>
            </div>
          </div>
        </div>
        {{-- <!-- END ARTICLE --> --}}
