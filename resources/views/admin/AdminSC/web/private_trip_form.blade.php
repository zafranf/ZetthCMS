...
@section('content')
    <div class="panel-body">
        <form class="form-horizontal" action="{{ url(Session::get('current_url')) }}{{ isset($private->post_id)?'/'.$private->post_id:'' }}" method="post" enctype="multipart/form-data">
            {{ isset($private->post_id)?method_field('PUT'):'' }}
            {{ csrf_field() }}
            <div class="row">
                    ...
                    <div class="tab-content no-padding">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                        ...
                                        <div class="col-md-8">
                                            <div class="pwd-upload">
                                                <div class="pwd-upload-new thumbnail">
                                                    <img src="{!! _get_image_temp(isset($private->post_id)?"/assets/images/upload/".$private->post_cover:'', [560]) !!}">
                                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <input type="text" class="form-control" name="post_name" value="{{ isset($private->post_id)?$private->post_title:'' }}" autofocus onfocus="this.value = this.value;" maxlength="100" placeholder="Trip Name">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="post_description" class="col-md-4 control-label">Description</label>
                                        <div class="col-md-8">
                                            <textarea class="form-control" name="post_description" placeholder="Trip description" rows="5">{{ isset($private->post_id)?$private->post_content:'' }}</textarea>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <select id="meeting_point" name="meeting_point" class="form-control select2">
                                                ...
                                                @foreach($points as $point)
                                                    <option value="{{ $point->point_id }}" {{ (isset($private->post_id) && $private->meeting_point[0]->point_id==$point->point_id)?'selected':'' }}>{{ $point->point_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-md-8">
                                            <select id="duration" name="duration" class="form-control select2">
                                                ...
                                                @foreach($durations as $duration)
                                                    <option value="{{ $duration->duration_id }}" {{ (isset($private->post_id) && $private->duration[0]->duration_id==$duration->duration_id)?'selected':'' }}>{{ $duration->duration_name.' ('.$duration->duration_description.')' }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="col-sm-offset-4 col-sm-4">
                                            <div class="checkbox">
                                                <label>
                                                    <input type="checkbox" name="post_status" {{ (isset($private->post_status) && $private->post_status==0)?'':'checked' }}> Active
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <div id="d_shedule">
                                                    ...
                                                    @if(isset($private->post_id))
                                                        @foreach($day[1] as $n => $schedule)
                                                            ...
                                                            <div class="col-md-2 no-padding"><input type="text" name="start[]" class="form-control no-radius timepicker" placeholder="Time" value="{{ isset($private->post_id)?$schedule->schedule_start:date('H') }}"></div>
                                                            <div class="col-md-2 no-padding" style="display:none;"><input type="text" name="end[]" class="form-control no-radius timepicker" placeholder="Until" value="{{ isset($private->post_id)?$schedule->schedule_end:date('H', strtotime('+1 hour')) }}"></div>
                                                            <div class="col-md-{{ ($n>1)?8:9 }} no-padding"><input type="text" name="activity[]" class="form-control no-radius-left" placeholder="Activity" value="{{ isset($private->post_id)?$schedule->schedule_activity:'' }}"><input type="hidden" name="day[]" value="1"></div>
                                                            @if($n>1)
                                            ...
                                            <div id="day{{ $b }}" class="tab-pane">
                                                <div id="d_show_schedule{{ $b }}" class="col-md-6" style="margin-top:10px;">
                                                    ...
                                                    <div id="d_shedule{{ $b }}">
                                                        ...
                                                        <div class="col-md-2 no-padding" style="padding-left: 10px; display:none;"><b>Until</b></div>
                                                        <div class="col-md-9 no-padding" style="padding-left: 10px;"><b>Activity</b></div>
                                                        @if(isset($private->post_id) && isset($day[$b]))
                                                            @foreach($day[$b] as $n => $schedule)
                                                                ...
                                                                <div class="col-md-2 no-padding"><input type="text" name="start[]" class="form-control no-radius timepicker" placeholder="Time" value="{{ isset($private->post_id)?$schedule->schedule_start:date('H') }}"></div>
                                                                <div class="col-md-2 no-padding" style="display:none;"><input type="text" name="end[]" class="form-control no-radius timepicker" placeholder="Until" value="{{ isset($private->post_id)?$schedule->schedule_end:date('H', strtotime('+1 hour')) }}"></div>
                                                                <div class="col-md-8 no-padding"><input type="text" name="activity[]" class="form-control no-radius-left" placeholder="Activity" value="{{ isset($private->post_id)?$schedule->schedule_activity:'' }}"><input type="hidden" name="day[]" value="{{ $b }}"></div>
                        ...
                        <div id="price" class="tab-pane">
                            <div class="row">
                                ...
                                <div class="col-md-7">
                                    <a id="btn-add-price" class="btn btn-default" title="Add price detail"><i class="fa fa-plus"></i> Add</a>
                                    <div id="d_price" style="margin-top:10px;">
                                        {{-- Form Header --}}
                                        <div class="col-md-1 no-padding" style="padding-left: 10px;"><b>No.</b></div>
                                        <div class="col-md-2 no-padding" style="padding-left: 10px;"><b>Min. Person</b></div>
                                        <div class="col-md-2 no-padding" style="padding-left: 10px;"><b>Max. Person</b></div>
                                        <div class="col-md-7 no-padding" style="padding-left: 10px;"><b>Price</b></div>
                                        @if(isset($private->post_id))
                                            @foreach($private->price as $n => $price)
                                                <div id="price{{ ++$n }}"><div class="col-md-1 no-padding"><input type="text" class="form-control no-radius-right text-center" value="{{ $n }}" readonly></div>
                                                <div class="col-md-2 no-padding">
                                                    <select name="minimal_person[]" id="minimal_person" class="form-control no-radius">
                                                        @for($i=1;$i<=20;$i++)
                                                            <option {{ (isset($private->post_id) && $price->price_min==$i)?'selected':'' }}>{{ $i }}</option>
                                                        @endfor
                                                        <option {{ (isset($private->post_id) && $price->price_min==">")?'selected':'' }}>&gt;</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-2 no-padding">
                                                    <select name="maximal_person[]" id="maximal_person" class="form-control no-radius">
                                                        @for($i=1;$i<=20;$i++)
                                                            <option {{ (isset($private->post_id) && $price->price_max==$i)?'selected':'' }}>{{ $i }}</option>
                                                        @endfor
                                                    </select>
                                                </div>
                                                <div class="col-md-{{ ($n>1)?6:7 }} no-padding"><input type="text" name="price[]" class="form-control no-radius-left" placeholder="Price" value="{{ isset($private->post_id)?$price->price_price:'' }}"></div>
                                                @if($n>1)
                                                <div class="col-md-1 no-padding"><center><i style="cursor:pointer;" onclick="_remove('#price{{ $n }}')" class="fa fa-minus form-control no-radius-left" title="delete this price"></i></center></div>
                                                @endif
                                                </div>
                                            @endforeach
                                        @else
                                            ...
                                            <div class="col-md-7 no-padding"><input type="text" name="price[]" class="form-control no-radius-left" placeholder="Price"></div>
                                        @endif
                                    </div>
                                </div>
                            </div>
...
var no_schedule = {{ isset($private->post_id)?count($day[1]):1 }};
@for($s=2;$s<=7;$s++)
    var no_schedule{{ $s }} = {{ (isset($private->post_id) && isset($day[$s]) )?count($day[$s]):0 }};
@endfor
var no_price = {{ isset($private->post_id)?count($private->price):1 }};
